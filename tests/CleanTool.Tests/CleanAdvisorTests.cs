...
public class CleanAdvisorTests
{
    ...
    public async Task CleanAdvisorService_GeneratesPrioritizedRecommendations()
    {
        ...
        registry.Register(new CleanTool.Rules.Developer.GoBuildCacheRule());
    ...
    {
        var sandbox = Path.Combine(Path.GetTempPath(), "CleanTool_LeftoverTest_" + Guid.NewGuid().ToString("N"));
