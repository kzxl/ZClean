...
public class CleanAdvisorTests
{
    ...
    public async Task CleanAdvisorService_GeneratesPrioritizedRecommendations()
    {
        ...
        registry.Register(new ZeroClean.Rules.Developer.GoBuildCacheRule());
    ...
    {
        var sandbox = Path.Combine(Path.GetTempPath(), "ZeroClean_LeftoverTest_" + Guid.NewGuid().ToString("N"));
