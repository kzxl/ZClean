...
public class CleanerEngineTests
{
    ...
    {
        var sandbox = Path.Combine(Path.GetTempPath(), "ZeroClean_Test_Sandbox_" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(sandbox);

        try
        {
            var dummyFile = Path.Combine(sandbox, "dummy.tmp");
            await File.WriteAllTextAsync(dummyFile, "ZeroClean test payload");
